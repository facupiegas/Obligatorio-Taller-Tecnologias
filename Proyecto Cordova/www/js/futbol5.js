...
$('#nuevoPartido').live('swipeleft', function(){
    cargarPaginaJugarYa('slide');
});
$('#jugarYa').live('swiperight', function(){
    cargarPaginaNuevoPartido('left');
});
...
function inscribirsePartido(usuario, partido){
    $.ajax({
        ...
        success: function(retorno){
            if(retorno.retorno == 'OK'){
                var mp3 = document.getElementById("#tano");
                mp3.play();
                navigator.vibrate(400);
                return;
            }
...
function onSuccess(position){
    var longActual = position.coords.latitude;
    var latActual = position.coords.longitude;
    ajaxNoSeSiSeraDemasiado(longActual, latActual);
}
function onError(err){
    console.log(err);
}
function cargarPaginaJugarYa(){
    navigator.geolocation.getCurrentPosition(onSuccess, onError);
}

function ajaxNoSeSiSeraDemasiado(pLat, pLong){
    var canchaMasCerca;
        $.ajax({
            type: "GET",
            dataType: "json",
            url: "http://quierojugar.tribus.com.uy/noSeSiSeraDemasiado?latO=" + pLat + "&longO=" + pLong,
            success: function(retorno){
                if(retorno.retorno == 'OK'){
                   cargarDetalleCancha( retorno.canchas[0].nombre,"pop",true);
                }
            },
            error: function(err){
                //$('#detalleCancha #mensaje').html("<p>"+ JSON.parse(err.responseText) +"</p>")
...
function onSuccessFoto(imageURI) {
